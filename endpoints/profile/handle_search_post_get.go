...
func PostSearchGetHandler(c *fiber.Ctx) error {
	// * Parse user claims
	...
	// * Query p with sort by
	db := mod.DB.Preload("Owner")
	if query.SortBy == nil || *query.SortBy == enum.SortByDate {
		db = db.Where("owner_id =?", body.UserId).Order("created_at DESC")
	} else if *query.SortBy == enum.SortByLike {
		db = db.Where("owner_id = ?", body.UserId).Order("like_count DESC")
	}
